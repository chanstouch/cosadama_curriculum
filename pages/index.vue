...
<div>
  <div class="w-full bg-gradient-to-r from-slate-300 via-blue-200 to-slate-50">
      <Header />
      <div class="max-w-6xl py-28 px-20 mx-auto">
        <div class="text-2xl text-slate-600 font-semibold mb-9 leading-9 drop-shadow-sm">중앙대 비전공자 코딩 커뮤니티, <br> 코사다마의 <span class="underline-offset-2">오픈소스 커리큘럼</span>이 <br> 업로드되는 공간입니다.</div>
  ...
  <div class="max-w-6xl mx-auto pt-16 pb-24 px-16">
    <div class="text-2xl text-slate-600 font-semibold mb-10">커리큘럼</div>
    <div class="grid grid-cols-3 gap-x-6 gap-y-8">
      <div v-for='course of courses' :key="course">
        <nuxt-link :to='`${course.slug}/1`'>
          <div class="group">
            <div class="lead-box h-52 mb-5">
            ...
            <div>
              ...
              <div class="text-sm text-slate-400">커리 개발: {{course.authors}}</div>
            </div>
          </div>
        </nuxt-link>
      </div>
